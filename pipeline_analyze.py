# -*- coding: utf-8 -*-
"""
pipeline_analyze.py — Stage 2: 종목별 기술 분석
═══════════════════════════════════════════════════
[v20.1] collector.py 분해 — 직렬/병렬 분석 루프
"""
...
def analyze_universe(ctx: PipelineContext) -> PipelineContext:
    """Stage 2: 모든 종목에 대해 기술 분석 수행, df_out 생성"""
    from collector import (
        log, analyze_ticker, send_telegram_auto, MAX_WORKERS,
    )
    ...
    if not rows:
        ...
        if ctx.enable_telegram:
            send_telegram_auto(
                pd.DataFrame(), ctx.trade_ymd,
                market_summary="⚠️ 필터 통과 종목 없음 (휴장일/데이터부족)",
                limit_count=0,
            )
        ctx.df_out = None
        return ctx
