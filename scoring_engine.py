...
def calculate_structural_score(row) -> float:
    ...
    # → 국어 0점이면 총점 만점이어도 불합격
    gate_mult = pd.Series(1.0, index=df.index)

    # Gate 1: 거래량 품질(Vol_Quality) — 세력 참여 최소 확인
    vq_raw = _safe_col(df, 'Vol_Quality', 0.0)
    gate_mult *= np.where(vq_raw < 0.5, 0.3, np.where(vq_raw < 0.8, 0.6, 1.0))

    # Gate 2: MFI(자금 흐름) — 돈이 들어오고 있는가
    mfi_raw = _safe_col(df, 'MFI14', 50)
    gate_mult *= np.where(mfi_raw < 20, 0.3, np.where(mfi_raw < 30, 0.6, 1.0))

    # Gate 3: 거래대금 — 유동성 최소 보장
    tv = _safe_col(df, '거래대금(억원)', 0)
    if tv.sum() == 0:
        tv = _safe_col(df, '거래대금(억)', 0)
    gate_mult *= np.where(tv < 10, 0.2, np.where(tv < 30, 0.5, 1.0))
