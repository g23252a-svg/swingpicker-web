...
# [v52] UP 70 → 90. v40이 '검증 범위 밖 — 기존 유지'로 남겨둔 값을 처음 검정했고
#   근거가 전혀 없음이 확인됐다(레짐 재구성으로 UP 22일 확보 — market_regime.py
#   정의 그대로이고 저장 MARKET_REGIME과 겹치는 6일에서 100% 일치).
#   · UP 레짐 내부에서 알파 임계 스윕(60~95)의 한계기여: 80셀 중 66셀 음수,
#     유의 0셀, t 중위 -0.41(범위 -1.38~+0.51). 프로덕션 70의 한계기여
#     -0.147%p(t=-0.55, p=0.586, 21일).
#   · UP 내부 알파 기울기(95분위−60분위) = -0.07%p (t=-0.10) — 알파가 UP에서는
#     아예 판별하지 않는다. 반면 NEUTRAL은 +1.37%p(t=+2.77, rho=+0.98 단조).
#   · 최적 임계가 전·후반에서 60↔95로 왕복(서열 rho -0.55)하고 부호도 반전
#     (H1 -0.653%p / H2 +0.313%p) — 어떤 값도 노이즈 적합이다.
#   · 보유기간 FWD3/5/10 전부 음수(-0.22/-0.15/-0.65) — 지평 견고성 없음.
#   · 70이 추가로 들여보내는 밴드(UP 70~90)는 n=851 · 평균 -1.40% · 승률 31.6%로
#     무여과 기저(-1.86% · 30.8%)와 사실상 동급이다. 걸러내는 값이 없었다.
#   포트폴리오 수준에서도 평면90이 현행 차등을 4지평 × ALL/IS/OOS/에피소드
#   16칸 중 15칸에서 앞섰다(FWD5 OOS +0.239%p, t=+3.36, p=0.002).
#   수익 개선을 주장하지 않는다 — '70을 지지하는 근거가 없다'가 채택 사유다.
#   90은 DOWN과 같은 보수적 기본값이다.
# NEUTRAL 85 유지: v40 검증값이고, 90 상향은 OOS만 유의(+0.404%p t=+2.84)하고
#   IS는 음수(-0.154)라 프로젝트 규율상 단독 채택 불가.
# DOWN 90 유지: 80/85와 30셀 전부에서 구별 불가(유의 0셀)이나 95는 명확히 열화
#   (-0.461%p, 유의성 소실) → 국소 최적. 대안이 4지평 전부 음수 부호.
# 재검증 조건: UP 레짐이 60일(에피소드 20건) 이상 쌓이기 전에는 UP 임계에 대한
#   어떤 추가 튜닝도 노이즈 적합이다.
ALPHA_GATE_THRESHOLD = {
    "UP": 90.0,       # [v52] 70→90: 70을 지지하는 근거가 측정에 전무 (위 주석)
