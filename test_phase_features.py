...
test("ml_low 필드 존재", hasattr(DEFAULT_CONFIG, "ml_low"))
test("macro_weights 필드 존재", hasattr(DEFAULT_CONFIG, "macro_weights"))
test("to_weight_config_dict 삭제됨", not hasattr(DEFAULT_CONFIG, "to_weight_config_dict"))
test("snapshot 동작", isinstance(DEFAULT_CONFIG.snapshot(), dict))
test("snapshot_json 동작", isinstance(DEFAULT_CONFIG.snapshot_json(), str))
...
test("scoring_engine에 WEIGHT_CONFIG 변수 없음", not has_wc_var)
...
test("비활성(0일): timeout", r0.action == "timeout")
...
test("5일 움직임 부족: time_stop", r5.action == "time_stop")
test("time_stop reason 포함", "time_stop_5d" in r5.reason)
...
test("수익중 연장: time_stop 아님", r_ext.action != "time_stop")
...
test("SL 우선: stop_hit", r_sl.action == "stop_hit")

# 2e. build_trade_plan 내부 자동 주입 확인
from trade_plan import build_trade_plan
plan_auto = build_trade_plan(buy=10000, atr_val=300, last_c=10000)
test("build_trade_plan time_stop 자동주입", plan_auto.time_stop_days == DEFAULT_CONFIG.time_stop_days,
...
test("고유동(100억): base", s_high == cfg.slippage_base_bps, f"{s_high}")
test("중유동(10억) > 고유동", s_mid > s_high, f"{s_mid} vs {s_high}")
test("저유동(3억) > 중유동", s_low > s_mid, f"{s_low} vs {s_mid}")
test("None = 최대", s_none == cfg.slippage_base_bps * cfg.slippage_low_liq_mult)
test("0 = 최대", s_zero == s_none)
test("슬리피지 단조감소 (유동↑ → 슬리피지↓)", s_none >= s_low >= s_mid >= s_high)
...
test("트레일링 발동", r_trail.action == "trailing_stop")
test("트레일링 수익 양수", r_trail.return_pct > 0)

# 5b. 트레일링 OFF → 미발동
rule_off = ExecRule(trailing_stop_enabled=False)
r_off = exec_multi_bar(plan_t, bars_trail, rule=rule_off)
test("트레일링 OFF: 미발동", r_off.action != "trailing_stop")
...
test("TP 우선: tp_hit", r_tp_first.action == "tp_hit")
...
# 6a. 전략 선택
test("활황: breakout+pullback", set(select_strategies("NORMAL", 60)) == {"breakout", "pullback"})
test("보통: pullback", select_strategies("NORMAL", 45) == ["pullback"])
test("침체: mean_revert", select_strategies("NORMAL", 30) == ["mean_revert"])
test("CRITICAL: 관망", select_strategies("CRITICAL", 60) == [])

# 6b. 팩토리 생성
for name in StrategyFactory.available():
    strat = StrategyFactory.create(name)
    test(f"{name} 생성 OK", hasattr(strat, "filter") and hasattr(strat, "score"))
...
test("breakout 파이프라인: STRATEGY 컬럼", "STRATEGY" in picks.columns if not picks.empty else True)
test("breakout 파이프라인: STRATEGY_SCORE 컬럼",
...
test("방어 규칙 8개", len(ENTRY_DEFENSE_RULES) == 8)

r_normal = check_entry_defense({"gap_pct": 1, "ret_1d_%": 2, "RSI14": 55, "거래대금(억)": 50})
test("정상: enter", r_normal["action"] == "enter")

r_gap12 = check_entry_defense({"gap_pct": 13})
test("갭12%: hold", r_gap12["action"] == "hold")
...
test("Time Stop 경계: 4일(day5 미도달) → timeout", r_edge4.action == "timeout")
...
test("Time Stop 경계: +1.99% < 2.0% → time_stop", r_exact.action == "time_stop")
...
test("Time Stop 경계: +2.0% = threshold → timeout(통과)", r_pass.action == "timeout")
...
test("Time Stop 1일: +0.5% → time_stop", r_1d.action == "time_stop")

# -- 슬리피지 경계 --
# threshold 정확히 일치
s_at_threshold = estimate_slippage_bps(cfg.slippage_liq_threshold_eok, cfg)
test("슬리피지 threshold 정확히: base_bps", s_at_threshold == cfg.slippage_base_bps,
     f"{s_at_threshold} vs {cfg.slippage_base_bps}")

# threshold 바로 아래
s_just_below = estimate_slippage_bps(cfg.slippage_liq_threshold_eok - 0.01, cfg)
test("슬리피지 threshold-0.01: > base_bps", s_just_below > cfg.slippage_base_bps)

# 음수 거래대금
s_neg = estimate_slippage_bps(-10.0, cfg)
test("슬리피지 음수: 최대", s_neg == cfg.slippage_base_bps * cfg.slippage_low_liq_mult)

# 매우 큰 거래대금
s_huge = estimate_slippage_bps(100000.0, cfg)
test("슬리피지 10만억: base_bps", s_huge == cfg.slippage_base_bps)
...
test("Hard Block RSI=85.0: 통과 (gt, not gte)", len(p85) == 1)
...
test("Hard Block 전부 NaN: 처리 안 죽음", len(p_nan) + len(b_nan) == 1)
...
test("트레일링 trigger 정확히 3%: 활성→발동", r_te.action == "trailing_stop")
...
test("트레일링 trigger 미달(2.9%): 미발동", r_nt.action != "trailing_stop")
