...
try:
    ...
    st.error("⚠️ pykrx 모듈을 불러오는 데 실패했습니다. requirements.txt에 pykrx==1.0.51이 포함되어 있는지 확인해주세요.")
    st.stop()

# ---------------- 기본 설정 ----------------
st.set_page_config(page_title="Swing Picker Web v3.0.3a (LDY Edition)", layout="wide")

st.title("📈 Swing Picker Web v3.0.3a (KST Fix · LDY Edition)")
st.caption("거래대금 + 기술지표 기반 자동 스윙 종목 추천 (한국시간 대응 안정화판)")
...
def last_trading_day_kst(use_yesterday: bool = True):
    ...
    if use_yesterday:
        ...
        target = now - dt.timedelta(days=1) if now.hour < 16 else now
...
def normalize_ohlcv_columns(df: pd.DataFrame):
    if df is None or df.empty:
        return pd.DataFrame()
    mapping = {"open": "시가", "high": "고가", "low": "저가", "close": "종가", "volume": "거래량", "amount": "거래대금"}
...
def get_top_turnover_stocks(end, markets, top_turnover):
    ...
    if not all_df:
        return pd.DataFrame()
    df_all = pd.concat(all_df, ignore_index=False)
...
def analyze_stock(df: pd.DataFrame, code: str, name: str):
    ...
    try:
        ...
        _, _, hist = macd_series(c)
        macd_hist = hist.iloc[-1]
        if (ret5 <= DEFAULTS["RET5_MAX"]) and (ret10 <= DEFAULTS["RET10_MAX"]) and (vr >= DEFAULTS["VOL_RATIO_MIN"]) and (rsi < 70) and (macd_hist > 0):
            last = c.iloc[-1]
            return {
                "종목명": name,
                "종목코드": code,
                "현재가": f"{last:,.0f}",
                "추천매수가": f"{round(last*0.98):,}",
                "추천매도가": f"{round(last*1.10):,}",
                "5일수익률(%)": f"{ret5:.2f}",
                "10일수익률(%)": f"{ret10:.2f}",
                "거래량배수": f"{vr:.2f}",
                "RSI14": f"{rsi:.1f}",
...
if st.button("🔍 스캔 시작"):
    ...
    if top_df.empty:
        st.warning("⚠️ 거래대금 데이터를 불러올 수 없습니다.")
        st.stop()
