...
CHANGELOG: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "items": [
            ...
            "❌ **탈락시킨 후보(정직 보고):** ① **시장폭(MARKET_BREADTH) 진입 타이밍** — IS에서 전 임계값이 +0.72~+1.19%로 좋아 보였지만 OOS에서 전부 부호 반전(-1.33~-0.40), 시장폭 vs 향후 5일 수익 spearman +0.027(p=0.82). 사후 임계선택 과최적화로 확인해 폐기. ② **v41 섹터 모멘텀** — 세 축 모두 IC가 0과 구분되지 않는다(SECTOR_RS FWD5 IC -0.023 t=-1.22 / SECTOR_RANK +0.018 t=+0.90 / SECTOR_RET_5D -0.004 t=-0.17). 상위30% 엣지는 IS +0.94(t=+4.87) → OOS -0.21(t=-1.16)로 반전. 현재 라이브 사이징(0.5~1.5배)에 쓰이고 있어 기대수익 없이 분산만 키우는 상태 — 별건으로 중립화 검토(이 PR에 검증 미완 변경을 끼워넣지 않음).",
