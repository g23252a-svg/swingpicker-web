...
#
# [v49 재검증 — 유지 결정] 독립 패널(워크포워드 알파 포트폴리오 76일)에서 재현:
#   알파 틸트만 -0.22%/5일 → ×섹터모멘텀 -0.05%/5일
#   차이 +0.165%p · 페어드 t=+3.13 (p=0.0025) · 자본 -3.54% → -1.10%
#   단 효과크기는 초판 주장(+0.34%p)의 절반이고 후반 구간은 t=+1.31로 미유의.
#   변동성은 2.7% 증가 — 즉 공짜가 아니다.
# v46 창고에 적은 "기대수익 없이 분산만 키운다"는 서술은 틀렸다. 그 판정은
# 원시 축(SECTOR_RS/SECTOR_RANK/SECTOR_RET_5D)의 IC만 보고 내린 것인데,
# v41이 주장한 것은 처음부터 '강섹터×고알파' 상호작용이었다. 원시 축이
# 무예측인 것과 상호작용이 무효인 것은 다른 명제다 — 잘못된 대상을 검정했다.
# → 제거하지 않는다. 다만 효과 감쇠와 미유의한 후반 구간을 함께 기록해 둔다.
