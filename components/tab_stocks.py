    ...
    def _render_table(show):
        ...
        for _, r in show.iterrows():
            rows.append({
                ...
                "name": str(r.get("종목명", "—")),
                "elite": round(safe_float(r.get("ELITE_SCORE", 0))),
                "score": round(safe_float(r.get("DISPLAY_SCORE", 0))),
                "s": round(safe_float(r.get("STRUCT_SCORE", 0))),
                "t": round(safe_float(r.get("TIMING_SCORE", 0))),
                "m": round(safe_float(r.get("AI_SCORE", r.get("ML_SCORE", 0)))),
                "bal": round(safe_float(r.get("BALANCE_SCORE", 0))),
                "rr": round(safe_float(r.get("RR_NOW_TP1", 0)), 1),
