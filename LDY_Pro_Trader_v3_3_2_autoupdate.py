# -*- coding: utf-8 -*-
"""
LDY Pro Trader v3.4.0 (Auto Update + Top Picks + ERS + 신호등)
...
- Top Picks 패널: 최소 RR(= (T1-Entry)/(Entry-Stop)), 손절여유%, 목표1여유%, ERS(=EBS×RR) / Now 근접 밴드(±%)
"""
...
APP_TITLE = "📈 LDY Pro Trader v3.4.0 (Auto Update + Top Picks)"
...
num_cols_cast = ["종가","거래대금(억원)","시가총액(억원)","RSI14","乖離%","MACD_hist","MACD_slope","Vol_Z","ret_5d_%","ret_10d_%","EBS","추천매수가","추천매도가1","추천매도가2","손절가"]
for c in num_cols_cast:
    if c in latest.columns:
        latest[c] = pd.to_numeric(latest[c], errors="coerce")

# ────────────────────────────── 파생 메트릭 (RR/여유/ERS/근접) ──────────────────────────────
def add_trade_metrics(dfv: pd.DataFrame) -> pd.DataFrame:
    v = dfv.copy()
    # RR1 = (T1 - Entry)/(Entry - Stop)
    e  = pd.to_numeric(v.get("추천매수가"), errors="coerce")
    s  = pd.to_numeric(v.get("손절가"),   errors="coerce")
    t1 = pd.to_numeric(v.get("추천매도가1"), errors="coerce")

    denom = (e - s)
    rr1 = (t1 - e) / denom.replace(0, np.nan)
    rr1 = rr1.where((denom > 0) & (t1 > e), np.nan)

    # 여유(%) = (목표1 - 종가)/종가 *100, (종가 - 손절)/종가 *100
    c = pd.to_numeric(v.get("종가"), errors="coerce")
    tgt1_margin_pct  = (t1 - c) / c * 100
    stop_margin_pct  = (c - s) / c * 100

    # Now 근접 밴드 기준: (종가-추천매수가)/추천매수가 * 100 (절대값)
    now_diff_pct = (c - e) / e.replace(0, np.nan) * 100

    v["RR1"] = rr1
    v["목표1여유_%"] = tgt1_margin_pct
    v["손절여유_%"] = stop_margin_pct
    v["Now근접_%"] = now_diff_pct.abs()
    v["ERS"] = pd.to_numeric(v.get("EBS"), errors="coerce") * rr1
    return v

latest = add_trade_metrics(latest)
...
# ────────────────────────────── 🏆 Top Picks (ERS 랭킹 + 신호등) ──────────────────────────────
st.markdown("---")
st.header("🏆 Top Picks (ERS 랭킹 + 신호등)")

# 필터 컨트롤
c1,c2,c3,c4,c5,c6 = st.columns([1,1,1,1,1,1.2])
with c1:
    th_min_rr = st.slider("최소 RR(목표1/손절)", 0.00, 5.00, 1.00, 0.10)
with c2:
    th_stop = st.slider("손절여유 ≥ (%)", 0.00, 10.00, 0.00, 0.25)
with c3:
    th_tgt1 = st.slider("목표1여유 ≥ (%)", 0.00, 20.00, 0.00, 0.5)
with c4:
    th_ers = st.slider("ERS ≥ (EBS×RR)", 0.00, 8.00, 0.00, 0.25)
with c5:
    th_band = st.slider("Now 근접 밴드(±%)", 0.0, 3.0, 1.0, 0.1)
with c6:
    tp_topn = st.slider("표시 수", 5, 200, 50, 5)
...
# 파생 메트릭 포함
tp_cols_derived = ["RR1","손절여유_%","목표1여유_%","Now근접_%","ERS"]
for c in tp_cols_base + tp_cols_derived:
...
def traffic_light(row) -> str:
    conds = []
    # 각 조건 True/False 체크
    rr_ok   = row.get("RR1", np.nan)
    stop_ok = row.get("손절여유_%", np.nan)
    tgt_ok  = row.get("목표1여유_%", np.nan)
    ers_ok  = row.get("ERS", np.nan)
    band_ok = row.get("Now근접_%", np.nan)
    ebs_ok  = row.get("EBS", np.nan)

    conds.append(bool(not pd.isna(rr_ok)   and rr_ok  >= th_min_rr))
    conds.append(bool(not pd.isna(stop_ok) and stop_ok>= th_stop))
    conds.append(bool(not pd.isna(tgt_ok)  and tgt_ok >= th_tgt1))
    conds.append(bool(not pd.isna(ers_ok)  and ers_ok >= th_ers))
    conds.append(bool(not pd.isna(band_ok) and band_ok<= th_band))
    conds.append(bool(not pd.isna(ebs_ok)  and ebs_ok >= PASS_SCORE))

    score_pass = sum(conds)
    if score_pass == len(conds):
        return "🟢"
    if score_pass >= max(3, len(conds)-2):
        return "🟡"
    return "🔴"
...
# 필터 적용
tp = tp[
    (tp["RR1"].fillna(-999)   >= th_min_rr) &
    (tp["손절여유_%"].fillna(-999) >= th_stop) &
    (tp["목표1여유_%"].fillna(-999) >= th_tgt1) &
    (tp["ERS"].fillna(-999)   >= th_ers) &
    (tp["Now근접_%"].abs().fillna(999) <= th_band) &
    (tp["EBS"].fillna(-999)   >= PASS_SCORE)
]

# 정렬: ERS ▶ RR1 ▶ EBS ▶ 거래대금
tp = tp.sort_values(["ERS","RR1","EBS","거래대금(억원)"], ascending=[False,False,False,False]).head(int(tp_topn)).copy()
tp.insert(0, "신호등", tp.apply(traffic_light, axis=1))

# 숫자형 캐스팅(표시용)
for c in ["종가","추천매수가","손절가","추천매도가1","EBS"]:
    if c in tp.columns: tp[c] = pd.to_numeric(tp[c], errors="coerce").round(0).astype("Int64")
for c in ["거래대금(억원)","RR1","손절여유_%","목표1여유_%","Now근접_%","ERS"]:
    if c in tp.columns: tp[c] = pd.to_numeric(tp[c], errors="coerce")

tp_show_cols = ["신호등"] + tp_cols_base + tp_cols_derived
...
st.data_editor(
    ...
    num_rows="fixed",
    column_config={
        "신호등":        st.column_config.TextColumn(" "),
        "시장":         st.column_config.TextColumn("시장"),
        "종목명":       st.column_config.TextColumn("종목명"),
        "종목코드":     st.column_config.TextColumn("종목코드"),
        "근거":         st.column_config.TextColumn("근거"),
        ...
        "Now근접_%":      st.column_config.NumberColumn("Now근접(±%))",    format="%.2f"),
        "ERS":          st.column_config.NumberColumn("ERS(EBS×RR)",    format="%.2f"),
    },
)

st.download_button(
    "📥 Top Picks 다운로드 (CSV)",
    data=tp[tp_show_cols].to_csv(index=False, encoding="utf-8-sig"),
    file_name="ldy_top_picks.csv",
    mime="text/csv"
)

with st.expander("ℹ️ 지표/점수/메트릭 설명", expanded=False):
    st.markdown("""
**EBS(0~7)**: RSI 45~65 / MACD↑ / MA20±4% / VolZ>1.2 / MA20↑ / MACD>0 / 5d<10%  
**RR1**: (추천매도가1 − 추천매수가) ÷ (추천매수가 − 손절가)  
**손절여유%**: (종가 − 손절가) ÷ 종가 × 100  
**목표1여유%**: (추천매도가1 − 종가) ÷ 종가 × 100  
**ERS**: EBS × RR1 (신호의 질 × 보상비율)  
**Now 근접(±%)**: |종가 − 추천매수가| ÷ 추천매수가 × 100
""")
